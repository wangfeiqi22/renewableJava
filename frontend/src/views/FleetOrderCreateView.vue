          ...
          <div class="card order-card" style="padding: 0;">
            ...
            <el-form :model="orderForm" label-position="top" class="order-form" style="padding: 20px;">
              ...
              <div v-show="currentStep === 1">
                <!-- Driver Selection -->
                <div class="form-group driver-selection-group">
                  <label class="form-label">
                    <el-icon><Van /></el-icon>
                    选择司机 <span class="required">*</span>
                  </label>
                  <el-select
                  ...
                  >
                    <el-option
                      ...
                      :value="driver.id"
                      :disabled="isDriverFrozen(driver)"
                    >
                      <div class="driver-option">
                        <div class="driver-info">
                          <span class="driver-name">{{ driver.name }}</span>
                          <span class="driver-phone">{{ driver.phone || driver.phoneNumber || '' }}</span>
                        </div>
                        <el-tag v-if="isDriverFrozen(driver)" type="danger" size="small">已冻结</el-tag>
                        <el-tag v-else-if="isDriverAvailable(driver)" type="success" size="small">空闲</el-tag>
                        <el-tag v-else type="warning" size="small">{{ driver.status || '未知' }}</el-tag>
                      </div>
                    </el-option>
                  </el-select>
                  <div class="error-text" v-if="errors.driverId">{{ errors.driverId }}</div>
                </div>
...
// Helper function to check if driver is frozen (case-insensitive)
const isDriverFrozen = (driver) => {
  if (!driver || !driver.status) return false
  return driver.status.toLowerCase() === 'frozen' || driver.status === 'DISABLED'
}

// Helper function to check if driver is available
const isDriverAvailable = (driver) => {
  if (!driver || !driver.status) return true
  const status = driver.status.toLowerCase()
  return status === 'available' || status === 'active' || status === 'idle'
}

const loadDrivers = async (query = '') => {
  ...
  try {
    ...
    if (response.data) {
      ...
      if (Array.isArray(data)) {
        ...
        drivers.value = data.records
      } else {
        console.warn('Unexpected data structure:', data)
        drivers.value = []
      }
      console.log('Loaded drivers:', drivers.value)
    } else {
    ...
    ElMessage.error('加载司机列表失败: ' + (error.message || '请检查网络连接'))
  } finally {
...
const searchDrivers = async (query) => {
  ...
  driverLoading.value = true
  console.log('Searching drivers with query:', query)
  try {
    ...
    console.log('Search drivers API response:', response)
    ...
    if (response.data) {
      ...
      if (Array.isArray(data)) {
        ...
        console.warn('Unexpected search data structure:', data)
        drivers.value = []
      }
    } else {
      drivers.value = []
...
const validateStep1 = () => {
  ...
  if (!orderForm.driverId) {
    errors.driverId = '请选择司机'
    ElMessage.warning('请选择司机')
    return
  }
  
  const selectedDriver = drivers.value.find(d => d.id === orderForm.driverId)
  if (!selectedDriver) {
    errors.driverId = '请选择有效的司机'
    ElMessage.warning('请选择有效的司机')
    return
  }
  
  if (isDriverFrozen(selectedDriver)) {
    errors.driverId = '该司机已被冻结，无法指派'
    ElMessage.error('该司机已被冻结，无法指派')
    return
  }
  
  if (!orderForm.pickupAddress) {
    errors.pickupAddress = '请选择或填写清运地址'
...
const submitOrder = async () => {
  ...
  try {
    ...
    if (!selectedDriver) {
      ElMessage.error('请选择有效的司机')
      submitting.value = false
      return
    }
    
    if (isDriverFrozen(selectedDriver)) {
      ElMessage.error('该司机已被冻结，无法创建订单')
      submitting.value = false
      return
    }
    
    console.log('Submitting order with driver:', selectedDriver)
    ...
    const res = await api.post('/orders/fleet-create', payload)
    console.log('Order response:', res)
    
    if (res.data && res.data.orderNo) {
      ElMessage.success('🎉 订单提交成功！')
      router.push({ path: '/fleet/orders', query: { newOrderNo: res.data.orderNo, newOrderId: res.data.id } })
    } else {
...
.driver-selection-group .form-label {
  display: flex;
  align-items: center;
  gap: 8px;
  font-size: 16px;
...
.driver-selection-group .required {
  color: #f56c6c;
  font-size: 14px;
}
...
.driver-select :deep(.el-input__wrapper) {
  border-radius: 8px;
  padding: 14px 16px;
  box-shadow: none;
  border: 1px solid #e8e8e8;
...
.driver-select :deep(.el-input__wrapper:hover) {
  border-color: #667eea;
}

.driver-select :deep(.el-input__wrapper.is-focus) {
  border-color: #667eea;
