<template>
  <div class="profile-container">
    ...
    <div class="profile-content" v-if="user.id">
      <!-- User Basic Info -->
      <div class="user-card enterprise">
        <div class="user-avatar-wrapper">
          <el-avatar :size="64" :src="user.avatarUrl || 'https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png'" />
        </div>
        <div class="user-info">
          <div class="user-name">{{ user.companyName || user.username || '企业账户' }}</div>
          <div class="user-id">企业代码: EN-{{ user.id }}</div>
        </div>
      ...
      <div class="info-module">
        <div class="info-row">
          <div class="info-label"><el-icon><User /></el-icon> 联系人</div>
          <div class="info-value">{{ user.contactName || '未设置' }}</div>
        </div>
        <div class="info-row">
        ...
        <div class="info-row">
          <div class="info-label"><el-icon><Stamp /></el-icon> 企业认证</div>
          <div class="info-value">
            <span class="status-tag success" v-if="user.companyLicenseUrl">已认证</span>
      ...
      <!-- Functional Menu -->
      <div class="section-title">企业服务</div>
      <div class="menu-grid">
        ...
        <div class="menu-item" @click="router.push('/admin/sf')">
          <div class="menu-icon-box green">
            <el-icon><Van /></el-icon>
          </div>
          <div class="menu-name">车队调度</div>
        </div>
        <div class="menu-item" @click="router.push('/station/work')">
          <div class="menu-icon-box orange">
            <el-icon><HomeFilled /></el-icon>
          </div>
          <div class="menu-name">中转站</div>
        </div>
        <div class="menu-item" @click="router.push('/finance')">
          <div class="menu-icon-box purple">
            <el-icon><Money /></el-icon>
          </div>
          <div class="menu-name">财务对账</div>
        </div>
      </div>
...
import { ArrowLeft, Setting, Phone, User, Stamp, ArrowRight, Document, Van, HomeFilled, Money } from '@element-plus/icons-vue'
...
onMounted(() => {
  ...
  if (['individual', 'vip'].includes(user.value.role)) {
    ElMessage.warning('该页面仅限企业用户访问')
    router.replace('/home')
  }
})
